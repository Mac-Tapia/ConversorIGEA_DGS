...
def validate_dgs(
    ...
    *,
    ...
    if len(term_rows) != len(model.nodes):
        structural_errors.append(f'ElmTerm count {len(term_rows)} != source nodes {len(model.nodes)}')
    ...
    for cls, rows in (('ElmTerm',term_rows),('ElmLne',line_rows),('ElmLod',load_rows),('ElmXnet',source_rows),('ElmSubstat',substat_rows)):
        for r in rows:
    ...
    valid_objects = line_fids | load_fids | source_fids
    ...
    for load in model.loads:
        ...
        if len(cubs) == 1 and cubs[0].get('fold_id') != expected_term:
            connection_errors.append(f'{load.device_number}: load cubicle does not reference selected node {load.node_id}')
    ...
    if geography is not None:
        ...
        from .dgs import visible_pointterm_nodes
        ...
        electrical_graphic_objects = visible_term_fids | line_fids | load_fids | source_fids | sed_fids
        ...
        expected_graphics = len(visible_term_fids) + len(line_fids) + len(load_fids) + len(source_fids) + len(sed_fids)
        ...
        for fid in line_fids | load_fids | source_fids | sed_fids | visible_term_fids:
        ...
        for fid in load_fids | source_fids:
            gr = graphics_by_object.get(fid, [])
            ...
            if fid in load_fids and gr and gr[0].get('sSymNam') != 'd_load':
